...
public class MyController {
    ...
    /*Dame un url valido para este test-save */
    /*local */


   
    @GetMapping("/save-ticket/{name}")
    public String saveTicket(
            @PathVariable String name, // Nombre que se envía en la URL
            @RequestParam String responsable, // Responsable como parámetro de la URL
            @RequestParam String fechadeentrega // Fecha límite como parámetro de la URL
    ) throws Exception {
        String collectionName = "ticket"; // Colección configurada internamente
        
        Map<String, Object> data = new HashMap<>();
        data.put("name", name);
        data.put("responsable", responsable); // Guardar el responsable en el mapa
        data.put("fechadeentrega", fechadeentrega); // Guardar la fecha límite en el mapa

        firebaseService.saveData(collectionName, data); // El ID se genera automáticamente
        return "Ticket saved successfully for user: " + name;
